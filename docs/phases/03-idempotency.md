...
[ADR 003](../adr/003-redis-idempotency.md). The response rebuild must keep
FastAPI `BackgroundTasks` attached so receipt extraction still runs when a
key is present (`api/middleware/idempotency.py`).
